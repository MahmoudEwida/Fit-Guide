    ...
    if (
      ...
      if (angleChange > MIN_ANGLE_CHANGE_FOR_REP && !sessionComplete) {
        ...
        if (counter >= MAX_REPS_PER_SESSION && !sessionComplete) {
          ...
          console.log(`Session completed! ${MAX_REPS_PER_SESSION} reps finished.`);
...
function generateResultsFile() {
  ...
  const workoutData = {
    ...
    reps: {
      total: counter || 0,
      correct: correctReps || 0,
      incorrect: incorrectReps || 0,
    },
    formIssues: {
      elbowsOutward: improperElbowAngleCount || 0,
      tShapePushup: improperElbowAngleCount || 0,
      formNotStraight: 0,
    },
...
function showSessionComplete() {
  ...
  // Update statistics
  document.getElementById("totalReps").textContent = counter;
  document.getElementById("goodReps").textContent = correctReps;
  document.getElementById("badReps").textContent = incorrectReps;

  // Update form issues
  document.getElementById("elbowAngle").textContent = improperElbowAngleCount;
  document.getElementById("backNotStraight").textContent = 0; // Not implemented yet
  document.getElementById("bodyAlignment").textContent = 0; // Not implemented yet
