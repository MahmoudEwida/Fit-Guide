...
function saveSessionData(resultsObject) {
  try {
    ...
    console.log('Workout data saved to localStorage. Total sessions:', existingData.length);
...
function generateResultsFile() {
  ...
  const workoutData = {
    ...
    formIssues: {
      tooDeep: tooDeepCount || 0,
      notDeepEnough: notDeepEnoughCount || 0,
      kneesCaving: kneeCavingCount || 0
    },
    performanceScore: performanceScore,
    sessionId: Date.now().toString(36) + Math.random().toString(36).substr(2),
