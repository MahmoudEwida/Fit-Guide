...
let backTooStraightCount = 0;
let backTooTiltedCount = 0;
            ...
            if (repState === 'up' && elbowAngle > ELBOW_ANGLE_DOWN_THRESHOLD) {
                ...
                if (!repCounted && (performance.now() - lastRepTime) > MIN_TIME_BETWEEN_REPS) {
                    ...
                    if (hasError && !currentRepHasError) {
                        ...
                        // Increment specific error counters based on the actual errors
                        if (currentRepErrors.includes("Back too straight")) {
                            backTooStraightCount++;
                        }
...
function resetTrainer() {
    ...
    backTooStraightCount = 0;
    backTooTiltedCount = 0;
    currentRepHasError = false;
    repCounted = false;
    repState = 'up';
    exerciseState = 'waiting';
...
function showSessionComplete() {
    ...
    // Update form issues with correct counts
    document.getElementById('elbow-swinging').textContent = improperElbowAngleCount;
    document.getElementById('back-too-straight').textContent = backTooStraightCount;
    document.getElementById('back-too-tilted').textContent = backTooTiltedCount;
...
function calculatePerformanceScore() {
    ...
    // Base score from correct reps percentage (80% weight)
    const repScore = (correctReps / counter) * 80;

    // Form score based on the ratio of form issues to total reps (20% weight)
    const formIssuesRatio = improperElbowAngleCount / counter;
    const formScore = Math.max(0, 20 * (1 - formIssuesRatio));
...
function createResultsObject() {
    ...
    const workoutData = {
        ...
        formIssues: {
            ...
            backTooTilted: backTooTiltedCount,
            backTooStraight: backTooStraightCount
        },
    ...
    // If backupData is an array, append to it, otherwise create new array
    const dataToSave = Array.isArray(backupData) 
        ? [...backupData, workoutData]
        : [workoutData];

    // Save back to localStorage
    localStorage.setItem('workoutBackup', JSON.stringify(dataToSave));
    console.log('Workout data saved to localStorage:', workoutData);
...
function saveSessionData(resultsObject) {
    try {
        ...
        // Create the new workout data object
        const workoutData = {
        ...
        // Check for duplicate entries (sessions within the same minute)
        const currentTimestamp = new Date(`${resultsObject.date} ${resultsObject.time}`).getTime();
        const isDuplicate = existingData.some(entry => {
            const entryTimestamp = new Date(`${entry.date} ${entry.time}`).getTime();
            // Check if entries are within 60 seconds of each other and have the same workout name
            return Math.abs(currentTimestamp - entryTimestamp) < 60000 && 
                   entry.workoutName === workoutData.workoutName;
        });
