...
export default function WorkoutFeedBack() {
  ...
  useEffect(() => {
    console.log("Initializing WorkoutFeedBack component...");
    
    // Add loading timeout
    const loadingTimeout = setTimeout(() => {
      if (!workoutData) {
        setError("Loading timeout - No workout data found after 5 seconds. Please try again.");
      }
    }, 5000);
    ...
    if (!storedData) {
      setError("No workout data found. Please complete a workout first.");
    ...
    try {
      ...
      const workoutToUse = Array.isArray(parsedData) 
        ? parsedData[parsedData.length - 1] 
        : parsedData;

      if (!workoutToUse || !workoutToUse.workoutName) {
        setError("Invalid workout data structure.");
        clearTimeout(loadingTimeout);
        return;
      }

      setWorkoutData(workoutToUse);
      clearTimeout(loadingTimeout);
    } catch (error) {
